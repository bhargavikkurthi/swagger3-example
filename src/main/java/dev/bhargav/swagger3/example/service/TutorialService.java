package dev.bhargav.swagger3.example.service;

import dev.bhargav.swagger3.example.model.Tutorial;
...
public class TutorialService {
  ...
  public List<Tutorial> findByTitleContaining(String title) {
    return tutorials.stream()
            .filter(tutorial -> tutorial.getTitle().contains(title)).toList();
  }

  public Tutorial findById(long id) {
    return tutorials.stream()
            .filter(tutorial -> id == tutorial.getId()).findAny().orElse(null);
  }
  ...
  public Tutorial save(Tutorial tutorial) {
    ...
    if (tutorial.getId() != 0) {
      ...
      for (int idx=0; idx<tutorials.size(); idx++)
        if (tutorialId == tutorials.get(idx).getId()) {
